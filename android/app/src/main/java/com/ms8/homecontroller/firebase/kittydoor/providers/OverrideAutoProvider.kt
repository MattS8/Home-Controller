...
object OverrideAutoProvider {
    private const val TAG = "KittyDoor_AutoOverride"
    private fun getDatabaseReference() : DatabaseReference {
        return FirebaseDatabase.getInstance().reference
            ...
            .child(Constants.OVERRIDE_AUTO)
    }

    suspend fun getOverrideAuto(): Flow<Boolean> {
        return callbackFlow {
            val listenerRegistration = getDatabaseReference()
                .addValueEventListener(object : ValueEventListener {
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        cancel(message = "Cancelled Override Auto Listener - ${error.message}",
                            cause = error.toException())
                    }
                })
            awaitClose {
                Log.d(TAG, "Cancelling Kitty Door override auto listener")
